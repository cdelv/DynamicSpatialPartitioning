...
long int simulate_naive(const int N, const int MAX_DEPTH, const int steps, const float dt, const float k, const bool save, const bool print, const int SAVE_FREQUENCY, const std::string BASE_DIR, const float MAX_RADIUS, const float SPACING, const float polydispersity);
...
int main(int argc, char const *argv[])
{
    ...
    long int performance1, performance2, performance3;  
    ...
    std::cout << "poly,N,qtree,grid,naive" << std::endl;
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < 19; ++i) {
            int N = std::pow(2, i);
            ...
            if(N < 16384) {
                performance3 = simulate_naive(N, MAX_DEPTH, steps, dt, k, save, print, SAVE_FREQUENCY, BASE_DIR, MAX_RADIUS, SPACING, polydispersity);
                std::cout << polydispersity << "," << N << "," << performance1 << "," << performance2 << "," << performance3 << std::endl; 
            }
            else
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; ++j) {
                ...
                const float dy = spheres[i].y - spheres[j].y;
                const float distance = std::hypot(dx, dy);
                
                const float minDistance = spheres[i].r + spheres[j].r;
                if (distance < minDistance && distance > 0.0f) {                    
                    const float nx = dx / distance;
                    const float ny = dy / distance;
                    const float overlap = minDistance - distance;
